...
module MakeVersioned(Config:Config)(Atom:Rbset.ATOM) =
struct
 ...
 type targ = (K.t * (Atom.t * K.t)) 
  ...
  module IrminConvertTie =
    struct
      let () = ()
      let () = ()
      and (targ, madt) = let open Irmin.Type in mu2 (fun targ madt -> IrminConvert.mktarg madt, IrminConvert.mkmadt targ)
    end
  ...
  module Vpst :
    sig
      ...
      val with_init_version_do : RBset.t -> 'a t -> 'a
      val fork_version : ?parent:branch -> 'a t -> branch t
      val set_parent: branch -> unit t
      val get_latest_version : unit -> RBset.t t
      val sync_next_version : ?v:RBset.t -> RBset.t t
    ...
    struct
      ...
      let with_init_version_do (v : RBset.t) (m : 'a t) =
        Lwt_main.run
          ((BC_store.init ()) >>=
             (fun repo ->
