...
let _ = U.print_header "--------- MICROBLOG -------"
...
module CInit = MkConfig(struct let root = "/tmp/repos/microblog2.git" end)
module Imicroblog = Imicroblog.MakeVersioned(CInit)
module Vpst = Imicroblog.Vpst
...
let do_txn (fp: out_channel) i (pre: Microblog.db Vpst.t) 
                                      : Microblog.db Vpst.t =
  pre >>= fun db ->
  ...
  let db' = 
    try 
      if n<=2 then Db.do_follow_txn db 
      else if n <= 3 then Db.do_unfollow_txn db 
      else if n<=4 then Db.do_new_tweet_txn db 
      else Db.do_delete_follower_txn db 
    with Not_found -> 
