...
(* Maximum number od securities offered by all the trading companies *)
let _max_securitys = 500
(* Maximum number of companies available *)
let _max_comapnies = 500
(* Maximum number of customers *)
let _max_customers = 1000
(* Maximum number of account *)
let _max_accounts = 5000
(* Maximum number of brokers *)
let _max_brokers = 100
(* Customer per broker *)
let _customer_per_broker = 10
...
let load_customer c ct ctier = 
  let open Customer in 
  let c_id = Id.of_int c in 
  let c_tax_id = Id.of_int ct in 
  let c_tier = Random.int64 3L in
  let _ = if (c+1) mod 1000 = 0 then 
       (printf "Customer (%d, %d) loaded\n" (c+1);
        flush_all()) in 
  T.Insert.customer_table {c_id; c_tax_id; c_tier}
 ...
 let load_customer_account caid cabid cacid = 
   ...
   let ca_bal = 10L in 
   let _ = if (caid+1) mod 500 = 0 then 
