...
module CInit = MkConfig(struct let root = "/tmp/repos/tpce2.git" end)
module Itpce = Itpce.MakeVersioned(CInit)
module Vpst = Itpce.Vpst
...
let do_txn (fp: out_channel) i (pre: Tpce.db Vpst.t) 
                                      : Tpce.db Vpst.t =
  pre >>= fun db ->
  ...
  let db' = 
    try 
      if n<=1 then Db.do_trade_result_txn db 
      else if n<=2 then Db.do_trade_cleanup_txn db    
      else if n<= 3 then Db.do_trade_order_txn db
      else if n<= 4 then Db.do_market_feed_txn db 
      else Db.do_trade_update_txn db 
    with Not_found -> 
