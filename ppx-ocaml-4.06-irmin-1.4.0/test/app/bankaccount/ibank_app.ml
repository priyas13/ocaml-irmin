...
module MakeVersioned (Config: Config) = struct
  ...
  type vt = {acct_id:int64; value:int64}
  ...
  module M = struct
    module AO_value = struct
      ...
      let vt = 
        ...
        record "node" (fun acct_id value -> {acct_id;value})
        |+ field "acct_id" int64 (fun t -> t.acct_id)
        |+ field "acct_id" int64 (fun t -> t.acct_id)
        |> sealr
    ...
    let rec of_adt (a:OM.t) : t Lwt.t  =
      ...
      aostore_add =<<
      (match a with
       | OM.{acct_id=t;value=v} -> 
          Lwt.return {acct_id= Int64.of_int t; value=Int64.of_int v})
    ...
    let rec to_adt (k:t) : OM.t Lwt.t =
      ...
      (match t with
      | {acct_id=t;value=v} ->
         Lwt.return {OM.acct_id= Int64.to_int t; OM.value=Int64.to_int v})
