...
module MakeVersioned (Config: Config) (Atom: Lww_app.ATOM) = struct
  module OM = Lww_app.Make(Atom)
  module K = Irmin.Hash.SHA1

  let from_just = function (Some x) -> x
  | None -> failwith "Expected Some. Got None."

  (* vreg is a record type *)
  type vreg = {time:int64; value:OM.atom}
  ...
  module M = struct
    module AO_value = struct
      ...
      let vreg = 
        ...
        record "node" (fun time value -> {time;value})
        |+ field "time" int64 (fun t -> t.time)
        |+ field "value" Atom.t (fun t -> t.value)
        |> sealr
    ...
    let rec of_adt (a:OM.t) : t Lwt.t  =
      ...
      (match a with
       | OM.{time=t;value=v} -> 
          Lwt.return {time= Int64.of_int t; value=v})
    ...
    let rec to_adt (k:t) : OM.t Lwt.t =
      ...
      (match t with
      | {time=t;value=v} ->
         Lwt.return {OM.time= Int64.to_int t; OM.value=v})
