...
module MakeVersioned (Config: Config) (Atom : Microblog.ATOM)  = struct
  module U = Microblog.User
  module T = Microblog.Timeline
  type t = {user_table : U.t list; timeline_table : T.t list}
  ...
  module AO_value : Irmin.Contents.Conv with type t = t = struct
    ...
    let t = let open Irmin.Type in
                (((record "madt"
                     (fun user_table -> fun timeline_table -> { user_table; timeline_table}))
                    |+
                    (field "user_table" (list U.t) (fun t -> t.user_table)))
                    |+ (field "items" (list T.t) (fun t -> t.timeline_table)))
                  |> sealr
    ...
    let of_string s =
        ...
        module AO_store =
          struct
            ...
            let rec add_adt t (a : OM.t) =
              ((add t) =<<
                 (match a with
                  | { user_table; timeline_table} -> Lwt.return @@ { user_table; timeline_table }) : 
              K.t Lwt.t)
            let rec read_adt t (k : K.t) =
              ((find t k) >>=
                 (fun aop ->
                    ...
                    match a with
                    | { user_table; timeline_table} ->
                        Lwt.return @@
                          { OM.user_table = user_table; OM.timeline_table = timeline_table }) : 
              OM.t Lwt.t)
          end
        ...
        module BC_value =
          (struct
             ...
             let of_adt (a : OM.t) =
               ((AO_store.create ()) >>=
                  (fun ao_store ->
                     ...
                     match a with
                     | { user_table; timeline_table} ->
                         Lwt.return @@ { user_table; timeline_table}) : t Lwt.t)
             let to_adt (t : t) =
               ((AO_store.create ()) >>=
                  (fun ao_store ->
           ...
           end : (IRMIN_STORE_VALUE with type  t =  t))
        module BC_store =
          struct
