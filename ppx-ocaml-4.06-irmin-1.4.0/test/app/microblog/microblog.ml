...
module Tweet = 
 ...
 let t = let open Irmin.Type in
                (((record "madt"
                     (fun tweet_id -> fun author_name -> fun content -> { tweet_id; author_name; content}))
                    |+
                    (field "tweet_id" User_id_type.t (fun t -> t.tweet_id)))
                    |+ (field "author_name" Irmin.Type.string (fun t -> (t.author_name)))
                    |+ (field "content" Irmin.Type.string (fun t -> (t.content))))
                  |> sealr
...
module Timeline = 
 ...
 let t = let open Irmin.Type in
                (((record "madt"
                     (fun id -> fun tl -> { id; tl}))
                    |+
                    (field "id" User_id_type.t (fun t -> t.id)))
                    |+ (field "tl" (list Tweet.t) (fun t -> (t.tl))))
                  |> sealr
...
module User = 
 ...
 let t = let open Irmin.Type in
                (((((((record "madt"
                     (fun id -> fun name -> fun followers -> 
                     fun following -> fun user_home -> { id; name; followers; following; user_home}))
                    |+
                    (field "id" User_id_type.t (fun t -> t.id)))
                    |+
                    (field "name" Irmin.Type.string (fun t -> t.name)))
                    |+
                    (field "followers" (list User_id_type.t) (fun t -> t.followers)))
                    |+
                    (field "following" (list User_id_type.t) (fun t -> t.following)))
                    |+
                    (field "user_home" (list Tweet.t) (fun t -> t.user_home))))
                  |> sealr
...
module Microblog = 
 ...
 type t = {user_table : User.t list; timeline_table : Timeline.t list}
