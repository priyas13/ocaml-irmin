...
module OM = Shoppingcart_app.Shoppingcart_app
...
module MakeVersioned (Config: Config) (Atom : Shoppingcart_app.ATOM)  = struct
  module Atom = Shoppingcart_app.Atom

  type t = {cart_id : string; items : Atom.t list}
  ...
  module AO_value : Irmin.Contents.Conv with type t = t = struct
    ...
    let t = let open Irmin.Type in
                (((record "madt"
                    ...
                    |+ (field "items" (list Atom.t) (fun t -> t.items)))
                  |> sealr
            ...
            let rec read_adt t (k : K.t) =
              ((find t k) >>=
                 (fun aop ->
                    ...
                    | { cart_id; items;_} ->
                        Lwt.return @@
                          { OM.cart_id = cart_id; OM.items = items }) : 
              OM.t Lwt.t)
          end
