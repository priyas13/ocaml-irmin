...
module CInit = MkConfig(struct let root = "/tmp/repos/graph_imp.git" end)
module MInit = Igraph.MakeVersioned(CInit)(Graph_imp.Edge_type)
module H = Graph_imp.Make
...
let experiment_f (fp: out_channel) : unit =
  begin
    ...
    let real_mtime = !MInit.OG.merge_time in
