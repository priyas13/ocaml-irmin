...
let _ = 
  U.print_header "Merge6" in 

  let module Graph = Graph_imp.Make in

  let original = Graph.G((Graph.OS.Empty, Int64.of_int 7, "f", Graph.OS.Empty), E_G) in 
  ...
  let g2 = Graph.G((Graph.OS.Empty, Int64.of_int 1, "a", Graph.OS.Empty), 
                    Graph.G((Graph.OS.Empty, Int64.of_int 2, "b", (Graph.OS.add ("abb", Int64.of_int 1) Graph.OS.Empty)), E_G)) in  
