...
open OG
open OS
module ISet = Iset_imp.MakeVersioned(Config)(Atom)
open ISet
...
module G = Git_unix.Mem
   ...
   type in_edge = ISet.madt

   type out_edge = ISet.madt
  ...
  module IrminConvert =
    struct
      ...
      let (in_edge : in_edge Irmin.Type.t) = let open Irmin.Type in ISet.IrminConvertTie.madt
      let (out_edge : out_edge Irmin.Type.t) = let open Irmin.Type in ISet.IrminConvertTie.madt
      let context = let open Irmin.Type in pair in_edge (pair node (pair nlabel out_edge))
