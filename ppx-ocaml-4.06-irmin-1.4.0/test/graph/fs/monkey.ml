...
module Igraph =Igraph.MakeVersioned(CInit)(Graph_imp.Edge_type)(ISet.BC_value)
...
let do_an_insert_edge t = 
  ...
  let w = (Random.int64 Int64.max_int) in 
...
let loop_iter i (pre: GR.t Vpst.t) : GR.t Vpst.t = 
  pre >>= fun t ->
  let t1 = Sys.time() in
...
let experiment_f ((*fp: out_channel*)) : unit =
  begin
    ...
    printf "Done\n";
    let kb = (!avg_tree_size * 420)/1024 in
    printf "%d\n" kb;
    (*
