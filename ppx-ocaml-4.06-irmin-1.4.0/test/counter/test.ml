...
let module MkConfig (Vars: sig val root: string end) : Derived_counter.ICounter.Config = struct
  let root = Vars.root
...
let module MInit = Derived_counter.ICounter.MakeVersioned(CInit) in 
let module M = Derived_counter.Counter in 
