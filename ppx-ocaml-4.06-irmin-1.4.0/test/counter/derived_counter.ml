      ...
      struct
        ...
        module AO_store =
          struct
            ...
            let rec add_adt t (a : Counter.t) =
              ((add t) =<< (Lwt.return @@ (Int64.of_int a)) : K.t Lwt.t)
            let rec read_adt t (k : K.t) =
              ((find t k) >>=
                 (fun aop ->
                    let a = from_just aop "to_adt" in
                    Lwt.return @@ (Int64.to_int a)) : Counter.t Lwt.t)
          end
        module type IRMIN_STORE_VALUE  =
          sig
            include Irmin.Contents.S
            val of_adt : Counter.t -> t Lwt.t
            val to_adt : t -> Counter.t Lwt.t
          end
        module BC_value =
          (struct
             include AO_value
             let of_adt (a : Counter.t) =
               ((AO_store.create ()) >>=
                  (fun ao_store ->
                     let aostore_add adt = AO_store.add_adt ao_store adt in
                     Lwt.return @@ (Int64.of_int a)) : t Lwt.t)
             let to_adt (t : t) =
               ((AO_store.create ()) >>=
                  (fun ao_store ->
                     let aostore_read k = AO_store.read_adt ao_store k in
                     Lwt.return @@ (Int64.to_int t)) : Counter.t Lwt.t)
        ...
        module Vpst :
          sig
            ...
            val sync_next_version : ?v:Counter.t -> Counter.t t
