...
module MkConfig (Vars: sig val root: string end) : Derived_canvas.ICanvas.Config = struct
  let root = Vars.root
...
module MInit = Derived_canvas.ICanvas.MakeVersioned(CInit)
module M = Derived_canvas.Canvas
