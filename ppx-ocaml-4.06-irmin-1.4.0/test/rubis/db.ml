...
let load_item i = 
  ...
  let item_initialprice = Id.random 1000L in 
  let item_maxbid = 0L in 
  let item = {item_id; item_desc; i_name; i_price} in
...
let load_walletbid i j =
  ...
  Insert.walletbid_table {w_buy_id; w_b_id}
...
let load_penality i j =
  let open Penality in
  let u_p_id = Id.of_int i in
  let penality_amount = Id.of_int j in
...
let load_buyerwallet_account () =
  U.fold (fun i pre1 -> 
        pre1 >>= fun () -> load_buyerwallet i)
      10 (Txn.return ())

let load_sellerwallet_account () =
  U.fold (fun i pre1 -> 
        pre1 >>= fun () -> load_sellerwallet i)
      300 (Txn.return ())
...
let do_deposit_to_wallet_txn db =
  ...
  snd @@ deposit_to_wallet_txn wid amt 
...
let do_new_bid_txn db = 
  ...
  snd @@ new_bid_txn bid wid iid 

let do_get_maxbid_for_an_item_txn db = 
  let iid = Id.random 300 in 
  snd @@ get_maxbid_for_an_item_txn iid 

let do_auction_txn db = 
  let sid = Id.random 300 in 
  let iid = Id.random 300 in 
  snd @@ auction_txn sid iid 

let do_update_penality_for_buyers_txn db = 
  let bid = Id.random 10 in 
  snd @@ update_penality_for_buyers_txn bid
...
let dump_keys db = 
  ...
  begin 
    fprintf fp "\n> Item\n";
    WarehouseTable.iter dump1 db.item_table;
    fprintf fp "\n> BuyerWallet\n";
    DistrictTable.iter dump1 db.buyerwallet_table;
    fprintf fp "\n> SellerWallet\n";
    OrderTable.iter dump1 db.sellerwallet_table;
    fprintf fp "\n> Bid\n";
    NewOrderTable.iter dump3 db.bid_table;
    fprintf fp "\n> ItemBid\n";
    OrderLineTable.iter dump2 db.itembid_table;
    fprintf fp "\n> WalletBid\n";
    ItemTable.iter dump2 db.walletbid_table;
    fprintf fp "\n> WalletItem\n";
    HistTable.iter dump2 db.walletitem_table;
    fprintf fp "\n> Penality\n";
    StockTable.iter dump1 db.penality_table;
    close_out fp;
