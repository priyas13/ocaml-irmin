...
let _ =
  U.print_header "List";
let module MkConfig (Vars: sig val root: string end) : List_derived.IList.Config = struct
  let root = Vars.root
...
let thread2_f : unit Vpst.t = 
  ...
  let c0' = (MInit.OM.set c0 4 (Int64.of_int 40))   in
  Vpst.sync_next_version ~v:c0' >>= fun c1 ->
  Vpst.liftLwt @@ Lwt_unix.sleep 0.5 >>= fun () ->
  let c1' = (MInit.OM.set c1 2 (Int64.of_int 60))  in 
 ...
 let thread1_f : unit Vpst.t = 
  ...
  let c0' = (MInit.OM.set c0 1 (Int64.of_int 10)) in
  Vpst.sync_next_version ~v:c0' >>= fun c1 ->
  Vpst.liftLwt @@ Lwt_unix.sleep 0.1 >>= fun () ->
  let c1' = (MInit.OM.set c1 3 (Int64.of_int 20)) in
  Vpst.sync_next_version ~v:c1' >>= fun c2 ->
  let _ = Printf.printf "merged : %s\n" (U.string_of_list IntAtom.to_string c2) in 
  ...
  let main () =
   let original = [Int64.of_int 1; Int64.of_int 2; Int64.of_int 3; Int64.of_int 4; Int64.of_int 5] in
